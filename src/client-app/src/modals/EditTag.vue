    ...
    <Modal v-if="tag.model" title="Uredi oznaku" shown :onClose="toggle">
        <template #body>
            <form @submit.prevent="submit">
                <Text class="mb-3" label="Naziv" autoFocus v-model="tag.model.name" required
                    :error="tag.error?.errors?.name" />
                <IntegerBox class="mb-3" label="Slijed" v-model="tag.model.order" required
        ...
        <template #footer>
            ...
            <SpinButton class="btn-primary" :loading="tag.submitting" text="Spremi" loadingText="Spremanje"
                @click="submit" />
        </template>
    </Modal>
</template>