...
const getDateTimeLocalStrFromUtcStr = (dtStr: string) => {
    ...
    return `${String(dt.getFullYear()).padStart(4, '0')}-${String(dt.getMonth() + 1).padStart(2, '0')}-${String(dt.getDate()).padStart(2, '0')}T${String(dt.getHours()).padStart(2, '0')}:${String(dt.getMinutes()).padStart(2, '0')}:${String(dt.getSeconds()).padStart(2, '0')}`;
}
...
<template>
    <div>
        ...
        <input ref="el" class="form-control" :id="state.id" :class="{ 'is-invalid': error }" :placeholder="placeholder"
            :value="localStr" @input="update" :required="required" type="datetime-local">
