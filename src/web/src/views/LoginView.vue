                ...
                <div class="col col-sm-6 col-lg-8 col-xl-8">
                    ...
                    <template v-else>
                        ...
                        <form @submit.prevent="login">
                            <TextBox class="mb-3" label="Korisničko ime" autoComplete="username"
                                v-model="state.model.username" required autoFocus />
                            <TextBox class="mb-3" label="Lozinka" type="password" autoComplete="password"
                                v-model="state.model.password" required />
                            <p v-if="state.error" class="text-danger">{{ state.error.message }}</p>
                            <SpinButton class="btn-primary w-100 mb-3" :loading="state.submitting" text="Prijava"
                                loadingText="Prijava u tijeku" isSubmit />
                        </form>
                    </template>
                </div>
            </div>
