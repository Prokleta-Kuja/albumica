...
const clearComplete = (process?: boolean) => {
  ...
  if (process) UploadService.processQueue()
}
...
<template>
  ...
  <div v-else class="mb-3">
    ...
    <input
      ...
      id="add-to-queue"
      multiple
      accept="*.jpg,*.jpeg,*.png,*.mov,*.avi,*.mp4"
      @change="upload"
    />
  </div>
    ...
    <div class="accordion-item">
      ...
      <div id="danger-zone" class="accordion-collapse collapse" data-bs-parent="#danger-accordion">
        <div class="accordion-body">
          <button type="button" class="btn btn-danger w-50" @click="clearComplete(true)">
            Obradi ponovno
          </button>
          <button
            type="button"
            class="btn btn-warning w-50"
            @click="UploadService.reparseMissingCreated"
          >
            Parsiraj neuspjele
          </button>
        </div>
